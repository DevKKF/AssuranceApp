...
from oleasante import settings
# Create your views here.
...
def generate_modele_tarifs_excel(request, prestataire_id):
    ...
    filename = 'OLEASANTE_V2-TARIF_DU_PRESTATAIRE_' + slugify(str(prestataire.name)).upper() + '.xlsx'
...
def generate_modele_tarifs_bureau(request):
    ...
    filename = 'OLEASANTE_V2-TARIF_DU_BUREAU_' + slugify(str(request.user.bureau)).upper() + '.xlsx'
    pprint(filename)
