...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        print(tasks)
        print(len(tasks))
        send_dev_notification_background_task_mail("a.tissi@olea.africa", f'Demarrage Executions requêtes en background {len(tasks)} taches en attente')
        for task in tasks:
            try:
                ...
                try:
                    ...
                    print(e)
                # send_notification_background_task_mail("a.tissi@olea.africa", task)
                send_dev_notification_background_task_mail("a.tissi@olea.africa", str(e))

        self.stdout.write(self.style.SUCCESS('Executions requêtes terminé avec succès'))
        send_dev_notification_background_task_mail("a.tissi@olea.africa", 'Executions requêtes terminé avec succès')